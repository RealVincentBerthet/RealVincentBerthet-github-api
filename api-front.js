...
async function make(){
    ...
    //let repoList=getJson('tests/json/repolist.json');
    if(repoList.length>0){
        ...
        for(let i in repoList){
            ...
            //let contributorsList=getJson('tests/json/contributors.json');
            let contributors='';
            
            for(let c in contributorsList){
                contributors+=`<a href='${contributorsList[c].html_url}'><img style='width:64px;height:64px;border-radius: 50%;margin:5px;' src='${contributorsList[c].avatar_url}' class='rounded-circle' alt='${contributorsList[c].login}' title='${contributorsList[c].login}'></img></a>`;
            }
